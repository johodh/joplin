...
class sClient {
	...
	connect(editor) {
		...
		this.connection.onmessage = (e) => {
			...
			if (msg.type === 'newCollaborator') {
				const currentLine = 0;
				const newCollaborator = new Collaborator(msg.uid, msg.username, currentLine);
				...
				if (msg.cursorPos >= docLength) {
					const curzor = this.editor.getCursorPosition();
					for (let i = docLength; i <= msg.cursorPos; i++) {
	...
	connectionStatus(status = null) {
		if (!status) {
			return this.connectionStatuz;
		 } else {
			 this.connectionStatuz = status;
			 }
	}
	...
	enterKeyOnSelection() {
		...
		if (range.end.row !== range.start.row) {
			...
			const brokenRows = this.getBrokenRows(range, true);
			...
			const brokenRow2 = startRowText.slice(range.end.column,startRowText.length);
			this.preventUpdateOnNewLine = true;
	...
	delAndBspOnSelection() {
		...
		if (range.end.row !== range.start.row) {
			...
			const joinedRows = this.getBrokenRows(range);
			if (curzor.column === range.end.column && curzor.row === range.end.row) {
				this.preventUpdateOnNewLine = true;
				// brokenRow2 = this.removeInitialSpaces(destinationLine);
	...
	onKeyDownEvent = event => {
		...
		if (event.keyCode === 13 && this.enterKeyOnSelection()) {
			if (curzor.column === 0) {
				...
				const currentRowText = this.editor.session.getLine(curzor.row);
				const rowLength = currentRowText.length;
		...
		// backspace (8)
			...
			if (curzor.column === 0 && curzor.row > 0) {
				let prevRow = this.editor.session.getLine(curzor.row - 1);
